...
void Model::addModelColumn(const QList<QMap<QString, QString> > &smallMapsFromMapName, int columnToAddIndex, QStringList keysToTreat)
{
    beginInsertColumns(QModelIndex(),columnToAddIndex,columnToAddIndex);
    model->insertColumn(columnToAddIndex,QModelIndex());
    //model->appendColumn();
    endInsertColumns();
#warning ajouter la colonne et avec set data ajouter les données pour chaque ligne (elements de keysToTreat) avec les données recuperées dans smallMapsFromMapName
    //model->setData(model->index(i,columnToAddIndex),"",Qt::DisplayRole);
